...
\begin{center}
	...
	\begin{tikzpicture}
		\begin{feynman}
			...
			\diagram*{
				a --[scalar, out=135, in=-135, loop, min distance=1cm] a,
				a --[scalar, out=45, in=-45, loop, min distance=1cm] a,
			};
		\end{feynman}
	\end{tikzpicture}
\end{center}
...
\begin{center}
	\begin{tikzpicture}
		\begin{feynman}
			...
			\diagram*{
				a --[scalar] b[dot] --[scalar] c,
				b --[scalar, out=135, in=45, loop, min distance=1cm] b,
			};
		\end{feynman}
	\end{tikzpicture}
\end{center}
...
To understand how this works in momentum space, we consider the somewhat more exotic diagram
\begin{center}
	\begin{tikzpicture}
		\begin{feynman}
			\vertex (a);
			...
			\diagram*{
				a --[scalar] b[dot, label = $y$] --[scalar] c[dot, label = $z$] --[scalar] d,
...
\begin{center}
	\begin{tikzpicture}
		\begin{feynman}
			...
			\diagram*{
				a --[scalar, momentum = $p$] b[dot] --[scalar, momentum = $k_{2}$] c[dot] --[scalar, momentum = $q$] d,
				b --[scalar, half left, momentum = $k_{1}$] c,
				b --[scalar, half right, momentum' = $k_{3}$] c,
			};
		\end{feynman}
	\end{tikzpicture}
\end{center}
In this picture there are two undetermined momenta which we will have to integrate over. Taking the vertices to contribute $-i\lambda$ each and lines between the vertices to add momentum space propagators, we find
\begin{align*}
...
\paragraph{Pauli-Villars Regularization}
