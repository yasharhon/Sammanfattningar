...
Consider some theory desccribed by
\begin{align*}
	\lag = \frac{1}{2}\left(\del{}{\mu}\phi\del{\mu}{}\phi - m^{2}\phi^{2}\right) + \lag_{\text{int}},
\end{align*}
where the interaction term contains no derivatives and we have used the so-valled bare mass $m_{0}$. The equation of motion is
\begin{align*}
	(\dalem{} + m_{0}^{2})\phi = j(x),
\end{align*}
where $j(x)$ arises from the interaction term. We want to find a so-called asymptotic field $\phi_{\text{in}}$, which is a free Klein-Gordon field with mass $m$ that transforms the same as $\phi$ under the Poincare group. The two field can be related by adding a term $(m^{2} - m_{0}^{2})\phi$ to the equation of motion, which modifies the right-hand side to $\tilde{j}(x)$. This somehow implies that $\phi_{\text{in}}$ creates one-particle states from the physical vacuum $\ket{\Omega}$. This is not the same as the vacuum $\ket{0}$ of the free theory. We then find
\begin{align*}
