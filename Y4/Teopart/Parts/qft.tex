...
Pairing a lepton and its neutron in a doublet, we note that the vertex factor for an interaction vertex can be written in terms of this doublet, the gamma matrices and a Pauli matrix, which changes leptons to neutrinos and vice versa. Depending on which becomes which, the vertex factor is termed charge-raising or charge-lowering. We can also add the third Pauli matrix, corresponding to a neutral vertex. Now, because the vertex factors are now written in terms of generators of \SU{2}, the doublet may be taken to consist of fields acted on by a gauge group. The generators correspond to weak isospin. The neutral current we are talking about is not observable in itself, however, as it only couples fermions with the same chirality.