...
\end{align*}
for some (smooth) function $G$. We say that $G$ generates the symmetry.
...
Suppose now that we perform an infinitesimal transformation. We then obtain
\begin{align*}
	\dv{\ham}{\alpha} = \pob{\ham}{G}.
\end{align*}
We say that $G$ is a symmetry of the system if $\pob{\ham}{G} = 0$. We can show that if $G$ is a symmetry of the system, then it is conserved.

\paragraph{Something Lagrangians IG}
We return to the principle of least action. Symmetries of the Lagrangian were on the form $\lag\to\lag + \dv{F}{t}$. We can now choose $F = G$, and instead consider this as a variational problem on phase space instead, where we seek the extrema of
\begin{align*}
	S = \integ{}{}{t}{p_{i}\dot{q}_{i} - \ham}.
\end{align*}
We now say that a canonical transformation of the system is a transformation such that
\begin{align*}
	\integ{}{}{t}{P_{i}\dot{Q}_{i} - H}
\end{align*}
is still extremal under the transformation. The Euler-Lagrange equations imply that this is true if
\begin{align*}
