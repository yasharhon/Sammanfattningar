...
\paragraph{Other Types of Work}
The previous discussions have been focused on gaseous systems, where all work is manifest as expansions and compressions. When discussing the thermodynamics of other kinds of systems, we will need to add other work terms, of the form
\begin{align*}
	\din{W} = Y\dd{X},
\end{align*}
where $Y$ is an intensive generalized force and $X$ an extensive generalized displacement. For gases, we had $Y = -p$ and $X = V$. Other examples would include elastic rods, with $Y$ as the rod tension and $X$ as its extension, or liquid surfaces, with $Y$ as the surface tension and $X$ as the surface area.
