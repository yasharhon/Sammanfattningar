...
\begin{align*}
	UT_{b}U\adj = R_{b}^{a}T_{a},\ i(\del{}{\mu}U)U\adj = \alpha^{a}_{\mu}T_{a}
\end{align*}
we have
\begin{align*}
...
\begin{align*}
	&i(\del{}{\mu}U)U\adj = \del{}{\mu}\theta^{a}T_{a}(1 + i\theta^{a}T_{a}) \implies \alpha^{a}_{\mu} = \del{}{\mu}\theta^{a}, \\
...
\begin{align*}
	(B\p)_{\mu}^{a} = B_{\mu}^{b}\left(\kdelta{a}{b} + \theta^{c}\srconst{cb}{a}\right) - \del{}{\mu}\theta^{a} = B_{\mu}^{a} + \theta^{c}B_{\mu}^{b}\srconst{cb}{a} - \del{}{\mu}\theta^{a}.
\end{align*}
...
%TODO: Rename capital indices
We proceed by working with a normalization such that
\begin{align*}
	\tr(T_{a}T_{b}) = \frac{1}{2}\delta_{ab}.
\end{align*}
...
\end{align*}
The $u_{i}$ are amplitudes for each representative. A gauge transformation manifests as a mapping $u\to F(u, \theta)$ (assumed to have a trivial Jacobian), where the $\theta$ are the parameters of the gauge transformation. We must have $n > m = N^{2} - 1$, where $N$ is the dimension of the multiplet, for some reason. Assuming both $S$ and $O$ to be gauge invariant, we can generate the gauge slice by introducing $H_{A}(u)$, which break gauge invariance, and constants $B_{A}$, which are somehow representatives of each gauge. The $A$ is some index running from $1$ to $m$. Defining $\sigma_{A} = H_{A}(F(u, \theta)) - B_{A}$, we clearly have
\begin{align*}
...
\begin{align*}
	H(F(H(F(u, \theta_{1})), \theta_{2})) = H(F(u, \theta_{3}))
\end{align*}
...
\end{align*}
To go the final stretch we write
\begin{align*}
	\dd{\theta}_{A} = S_{AB}(\theta)\var{\theta_{C}}.
\end{align*}
$S$ is now the Jacobian of what is effectively a change of variables, and introducing $v = F(u, \theta)$ we are left with
\begin{align*}
...
\end{align*}
The former part is now dependent only on $\theta$ and disappears during the division. How nice. The Dirac deltas can be handled by introducing factors of
\begin{align*}
	\inte{}{}\prod\limits_{A = 1}^{m}\dd{B_{A}}e^{-\frac{i}{2\alpha}\sum\limits_{B = 1}^{m}B_{B}^{2}},
\end{align*}
as the $B$s and $H$s should not impact the physics. Introducing this we find the relevant parts of the numerator to be
\begin{align*}
